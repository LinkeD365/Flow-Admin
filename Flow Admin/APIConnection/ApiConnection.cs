    ...
    public partial class ApiConnection : Form
    {
        ...
        public HttpClient GetClient()
        {
            if (ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    graphConn = apiConns.GraphConns.FirstOrDefault(grph => grph.Id == (int)lblGraphName.Tag);
                    if (graphConn == null)
                    {
                        graphConn = new GraphConn();
                    }

                    graphConn.Id = (int)lblGraphName.Tag;
                    graphConn.TenantId = txtGraphTenant.Text;
                    graphConn.ReturnURL = txtGraphReturnURL.Text;
                    graphConn.AppId = txtGraphApp.Text;
                    graphConn.UseDev = chkGraphDev.Checked;
                    graphConn.Name = txtGraphName.Text;
                    var grpIndex = apiConns.GraphConns.IndexOf(graphConn);
                    if (grpIndex == -1)
                    {
                        apiConns.GraphConns.Add(graphConn);
                    }
                    else
                    {
